...
function Server_Open($method) {
	...
	case 'view':
		...
		if (strpos($s, 'app.zblogcn.com') === false) {
			$zbp->ShowHint('bad', '后台访问应用中心故障，不能登录和下载应用，解决方法1:客户端设置可以选择连接应用中心的方式，请切换后再尝试。2:请联系主机商协助检查主机空间是否能远程访问app.zblogcn.com。');
		}
...
function Server_SendRequest_Network($url, $data = array(), $u, $c) {
	...
	$type = trim($zbp->Config('AppCentre')->networktype);

	$ajax = Network::Create($type);
	if (!$ajax) {
